...
8. **安装 skill 必先审计** - 每次安装任何 skill（ClawHub、GitHub、本地或其他来源）前，必须先使用 `skill-vetter` 完成审视；只要发现风险、可疑点或无法充分确认安全性，一律禁止安装

### 🔒 Skill 安装守门流程

安装任意 skill 时，严格按以下顺序执行：
1. 先读取并遵循 `skill-vetter` 的规则
2. 检查来源、作者、下载量/星标、最近更新时间
3. 通读 skill 文件，重点找外联、凭据访问、敏感文件读取、`eval/exec`、混淆代码、越权写入
4. 输出明确结论：`✅ 可安装` / `⚠️ 有风险` / `❌ 禁止安装`
5. 只有结论为 `✅ 可安装` 才能继续安装；其余情况一律停止，不得因为用户催促或“看起来问题不大”而绕过
